...
namespace Cluster.GameMechanics.Universe
{
	class Shot
	{
		...
		public Shot(Unit from, Unit aim = null, byte inrange = 1)
		{
			...
			{
				r = 0.25f; g = 1.0f;b=0.4f;length=5.0f;
		...
		public Shot(Unit from, Planet pl, int index)
		{
			...
			r = owner.r * 0.25f + 0.75f; g = owner.g * 0.25f + 0.75f; b = owner.b * 0.25f + 0.75f; a = 0.5f;
			length = 25.0f;
			...
			y = (float)from.y;
			v = 75.0f;
			rot = (float)Math.Atan2(pl.y + (double)pl.size * 20.0 * Math.Sin(((double)index + 0.5) / (double)pl.size * 2.0 * Math.PI) - y,
									pl.x + (double)pl.size * 20.0 * Math.Cos(((double)index + 0.5) / (double)pl.size * 2.0 * Math.PI) - x);
		...
		void sim_units(float t)
		{
			...
			if (weapon_type != Prototype.WEAPON_LASER)
			{
				...
				for (int i = 0; i < Civilisation.count + 1; i++)
				{
					...
					foreach (Unit u in s.ships[i])
					{
						...
						else if (weapon_type == Prototype.WEAPON_EXPLOSIVE && dst < 30.0f * u.getPrototype().shape_scaling * u.getPrototype().shape.radius + 150.0f)
						{
			...
			{
				float mult = 1.0f;
				if (from != null && aim_unit.getPrototype().goodAgainst() == from.getPrototype().ship_class) mult = 0.15f;
		...
		void sim_buildings(float t)
		{
			...
			{
				removed.Add(this);
				int feld = (int)(((2.0 * Math.PI + Math.Atan2(dy, dx)) % (2.0 * Math.PI)) * ((float)aim_planet.size / (2.0 * Math.PI)));
				if (aim_planet.infra[feld] != null && aim_planet.infra[feld].owner != owner)
				{
					if (aim_planet.terra[feld] == Planet.TERRA_MOUNTAIN) { damage *= 0.5f; }
					else if (aim_planet.terra[feld] == Planet.TERRA_WATER) { damage *= 0.33f; }
					if (aim_planet.effect[feld] == Planet.EFFECT_RAIN) { damage *= 0.5f; } // Niederschlag/Wolken/Schneesturm reduziert Zielgenauigkeit und Schaden um 50%
			...
			foreach (List<Shot> slist in Shot.list)
			{
				foreach (Shot s in slist)
				{
					if (true)//Space.isVisible(s.x, s.y))//Math.Abs(s.x - cx) <= dx && Math.Abs(s.y - cy) <= dy)
					{
						...
						{
							render_buf_pos[render_shot_count * RENDER_ARRSIZE_POS + 0] = s.from.x + (float)Math.Cos(s.from.rot) * s.x - (float)Math.Sin(s.from.rot) * s.y;
							render_buf_pos[render_shot_count * RENDER_ARRSIZE_POS + 1] = s.from.y + (float)Math.Sin(s.from.rot) * s.x + (float)Math.Cos(s.from.rot) * s.y;
							...
							render_buf_alphas[render_shot_count * RENDER_ARRSIZE_ALPHAS + 0] = s.a * Math.Min(1.0f, s.lifespan);
							render_buf_alphas[render_shot_count * RENDER_ARRSIZE_ALPHAS + 1] = s.a * Math.Min(1.0f, s.lifespan);
						}
						else
						{
							render_buf_pos[render_shot_count * RENDER_ARRSIZE_POS + 0] = s.x;
							render_buf_pos[render_shot_count * RENDER_ARRSIZE_POS + 1] = s.y;
							render_buf_pos[render_shot_count * RENDER_ARRSIZE_POS + 2] = s.x - (float)Math.Cos(s.rot) * s.length;
							render_buf_pos[render_shot_count * RENDER_ARRSIZE_POS + 3] = s.y - (float)Math.Sin(s.rot) * s.length;
							render_buf_alphas[render_shot_count * RENDER_ARRSIZE_ALPHAS + 0] = 0.0f;
							render_buf_alphas[render_shot_count * RENDER_ARRSIZE_ALPHAS + 1] = s.a * Math.Min(1.0f, s.lifespan);
						}
						render_shot_count++;
						if (render_shot_count >= MAX_SHOT_RENDERINGS) { render_part2(); return; }
					}
				}
			}

			if (render_shot_count > 0) { render_part2(); }
		}
		static void render_part2()
		{
			...
			GL.BufferData(BufferTarget.ArrayBuffer, (render_shot_count + 1) * RENDER_ARRSIZE_POS *sizeof(float), render_buf_pos, BufferUsageHint.DynamicDraw);
			...
			GL.BufferData(BufferTarget.ArrayBuffer, (render_shot_count + 1) * RENDER_ARRSIZE_COL *sizeof(float), render_buf_col, BufferUsageHint.DynamicDraw);
			...
			GL.BufferData(BufferTarget.ArrayBuffer, (render_shot_count+1) * RENDER_ARRSIZE_ALPHAS * sizeof(float), render_buf_alphas, BufferUsageHint.DynamicDraw);
			...
			Space.shot_shader.bind();
			GL.Uniform3(Space.shot_shader.getUniformLocation("viewport"), GameWindow.active.mult_x, GameWindow.active.mult_y, Space.animation);
			GL.Uniform3(Space.shot_shader.getUniformLocation("scroll"), Space.scroll_x, Space.scroll_y, Space.zoom);
		...
		double turnTowards(float delta_x, float delta_y, double maxAngle = Math.PI)
		{
